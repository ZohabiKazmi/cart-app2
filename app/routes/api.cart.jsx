...
//           const results = await Promise.all(goals.map(goal => {
...
import { getDiscountTitle, getDiscountValue, validateDiscountData } from "./utils/discountHelpers";
...
export async function loader({ request }) {
  try {
    ...
    console.error("Error fetching spending goals:", error);
    return json(
      { success: false, error: "Failed to fetch spending goals" },
      { status: 500 }
    );
  }
}
...
export async function action({ request }) {
  ...
  try {
    switch (action) {
      case "CREATE": {
        ...
        const newGoalData = {
          ...
          percentageDiscount: formData.get("percentageDiscount") ? 
            parseFloat(formData.get("percentageDiscount")) : null,
          fixedAmountDiscount: formData.get("fixedAmountDiscount") ? 
            parseFloat(formData.get("fixedAmountDiscount")) : null,
        };

        // Validate the data
        validateDiscountData(newGoalData);

        // Create Shopify discount
        const discountResponse = await createAutomaticDiscount(session, {
          title: getDiscountTitle(newGoalData),
          spendingGoal: newGoalData.spendingGoal,
          discountType: newGoalData.selectedTab,
          discountValue: getDiscountValue(newGoalData)
        });
        ...
        // Store in database with Shopify discount ID
        const newGoal = await db.spendingGoal.create({
          data: {
            ...newGoalData,
      ...
      case "DELETE": {
        ...
        // Find the goal first
        const goalToDelete = await db.spendingGoal.findUnique({
          where: { id: deleteId }
        });
        ...
        // Delete Shopify discount if it exists
    ...
    console.error("Error managing spending goals:", error);
    return json(
      { success: false, error: error.message || "Failed to process operation" },
      { status: 500 }
    );
  }
}